...
router.get("/schema", async(req, res) => {
    const meta = await getMetaCol().findOne({ _session_id: req.sessionId }, { sort: { uploaded_at: -1 } });
    res.json(meta || null);
});
...
router.post("/upload", upload.single("file"), async(req, res) => {
    try {
        ...
        // Drop legacy indexes from before session isolation existed — they enforced
        // uniqueness across ALL sessions and would otherwise reject every row whose
        // _row_id / natural id happens to collide with data from another upload.
        for (const legacyName of["idx_row_id", "idx_natural_id"]) {
            await dataCol.dropIndex(legacyName).catch(() => {});
        }

        await dataCol.createIndex({ _session_id: 1, _row_id: 1 }, { name: "idx_session_row_id", unique: true });
        if (schema.id_column) {
            try {
                await dataCol.createIndex({ _session_id: 1, [schema.id_column]: 1 }, { name: "idx_session_natural_id", unique: true });
            } catch (e) {
