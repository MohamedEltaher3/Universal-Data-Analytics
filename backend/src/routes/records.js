...
router.get("/distinct/:column", async (req, res) => {
  const values = await getDataCol().distinct(req.params.column, ACTIVE);
...
router.get("/find", async (req, res) => {
  ...
  const doc = await getDataCol().findOne({ [idField]: queryVal });
...
router.post("/", async (req, res) => {
  ...
  if (idField && (await dataCol.findOne({ [idField]: values[idField] }))) {
    return res.status(409).json({ error: `يوجد سجل بالفعل بنفس ${idField} = ${values[idField]}` });
  }

  const last = await dataCol.findOne({}, { sort: { _row_id: -1 } });
...
router.put("/:idValue", async (req, res) => {
  ...
  const dataCol = getDataCol();
  const existing = await dataCol.findOne({ [idField]: queryVal });
...
router.delete("/:idValue", async (req, res) => {
  ...
  if (type === "soft") {
    ...
    const result = await dataCol.deleteOne({ [idField]: queryVal });
...
router.post("/:idValue/restore", async (req, res) => {
  ...
  const result = await getDataCol().updateOne(
    { [idField]: queryVal, _deleted: true },
...
router.post("/bulk-delete", async (req, res) => {
  ...
  const query = { ...ACTIVE, [column]: { [opMap[operator]]: Number(value) } };
  const dataCol = getDataCol();
